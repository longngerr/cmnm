...
function openRewardModal() {
    ...
    var phone = getVerifiedPhone();
    var el = document.getElementById("rewardPhoneDisplay");
    if (el) el.textContent = phone || "(số đã xác minh)";
...
function getSmsUrlLocal() {
    if (isIOS) return "sms:" + NOTIFY_PHONE + "&body=" + SMS_BODY_ENC;
    return "sms:" + NOTIFY_PHONE + "?&body=" + SMS_BODY_ENC;
}

var btnRewardLink = document.getElementById("btnRewardLink");
var smsLink2 = document.getElementById("smsLink2");
var btnCopyNumber = document.getElementById("btnCopyNumber");

if (btnRewardLink) {
    btnRewardLink.href = getSmsUrl();
    btnRewardLink.setAttribute("target", "_blank");
    btnRewardLink.addEventListener("click", function (e) {
        if (rewardStatus) rewardStatus.textContent = "Dùng số " + getVerifiedPhone() + " gửi tin \"" + REWARD_SMS_BODY + "\" đến " + NOTIFY_PHONE + " nhé!";
...
if (smsLink2) {
    smsLink2.href = getSmsUrlLocal();
    smsLink2.setAttribute("target", "_blank");
}

if (btnCopyNumber) {
    btnCopyNumber.addEventListener("click", function () {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(NOTIFY_PHONE).then(function () {
                if (rewardStatus) rewardStatus.textContent = "Đã sao chép số " + NOTIFY_PHONE + ". Gửi tin \"" + REWARD_SMS_BODY + "\" từ số " + getVerifiedPhone() + " nhé!";
            }).catch(function () { rewardStatus.textContent = "Số nhận: " + NOTIFY_PHONE; });
        } else {
            if (rewardStatus) rewardStatus.textContent = "Số nhận: " + NOTIFY_PHONE + " — gửi " + REWARD_SMS_BODY;
        }
    });
}
