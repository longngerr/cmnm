...
// Nút "Nhận thưởng" là link <a href="sms:..."> để tap trực tiếp mở app tin nhắn gốc (không bị chặn)
var REWARD_SMS_BODY = "Nhận";
var btnRewardLink = document.getElementById("btnRewardLink");
if (btnRewardLink) {
    btnRewardLink.href = "sms:" + NOTIFY_PHONE + "?body=" + encodeURIComponent(REWARD_SMS_BODY);
    btnRewardLink.addEventListener("click", function () {
        if (rewardStatus) rewardStatus.textContent = "Đã mở tin nhắn. Gửi tin \"" + REWARD_SMS_BODY + "\" đến " + NOTIFY_PHONE + " nhé!";
    });
}
